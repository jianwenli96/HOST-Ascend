...
class MoT(nn.Module):
    ...
    def _eager_attention_with_weights(
        self,
        q_cat: torch.Tensor,
        k_cat: torch.Tensor,
        v_cat: torch.Tensor,
        ...
        """Eager attention that returns both output and attention weights.
        Used on the last layer only, for attention stats logging.
        ...
        """
        attn_mask = attention_mask.to(device=q_cat.device)
        q = rearrange(q_cat, "b s (n d) -> b n s d", n=self.num_heads)
        k = rearrange(k_cat, "b s (n d) -> b n s d", n=self.num_heads)
        v = rearrange(v_cat, "b s (n d) -> b n s d", n=self.num_heads)

        scale = q.shape[-1] ** -0.5
        scores = torch.matmul(q, k.transpose(-2, -1)) * scale  # [B, H, S, S]
        # Apply boolean mask: blocked positions get -inf.
        # attn_mask is either [S, S] or [B, 1, S, S]; broadcast to [B, H, S, S].
        if attn_mask.ndim == 2:
        ...
        output = rearrange(output, "b n s d -> b s (n d)", n=self.num_heads)
    ...
    def _apply_expert_post_block(
        ...
        cross_attn_weights_store: Optional[list] = None,
    ) -> torch.Tensor:
        x = block.gate(residual_x, gate_msa, block.self_attn.o(mixed_attn_out))

        if context_payload is not None:
            # Task video cross-attention (before text), with RoPE on Q and K
            tv_context = context_payload.get("task_video_context")
            if tv_context is not None and block.task_video_cross_attn is not None:
                tv_mask = context_payload.get("task_video_mask")
                if tv_mask is not None and tv_mask.dim() == 3:
                    tv_mask = tv_mask.unsqueeze(1)
                q_freqs = context_payload.get("q_freqs")
                k_freqs = context_payload.get("task_video_freqs")
                tv_out, _ = block.task_video_cross_attn(
                    block.norm_task_video(x), tv_context, ctx_mask=tv_mask,
                    q_freqs=q_freqs, k_freqs=k_freqs,
                )
                # Zero cross-attn output for dropped task_video samples
                # to prevent o.bias leak when attention mask is all-False.
                tv_dropped = context_payload.get("task_video_dropped_mask")
                if tv_dropped is not None:
                    _keep = torch.tensor(
                        [not d for d in tv_dropped],
                        dtype=tv_out.dtype, device=tv_out.device,
                    ).view(-1, 1, 1)
                    tv_out = tv_out * _keep
                x = x + tv_out

            # prepend_cross_attn mode: video expert self-attends to its own [p|tv] dynamic context.
            # Action expert never enters here (no "prepend_cross_attn_slice" in its context_payload).
            pac_slice = context_payload.get("prepend_cross_attn_slice")
            if pac_slice is not None and block.task_video_cross_attn is not None:
                s, e = pac_slice
                tv_context_dyn = x[:, s:e, :]   # [B, p+tv, D] — extracted from post-gate x
                tv_mask_dyn = context_payload.get("task_video_mask")
                if tv_mask_dyn is not None and tv_mask_dyn.dim() == 3:
                    tv_mask_dyn = tv_mask_dyn.unsqueeze(1)
                q_freqs_dyn = context_payload.get("q_freqs")
                k_freqs_dyn = context_payload.get("task_video_freqs")
                tv_out_dyn, _ = block.task_video_cross_attn(
                    block.norm_task_video(x), tv_context_dyn, ctx_mask=tv_mask_dyn,
                    q_freqs=q_freqs_dyn, k_freqs=k_freqs_dyn,
                )
                # Zero out task-video cross-attn for noisy group tokens to avoid
                # all-False mask softmax artifact (uniform weights → non-zero output).
                # Ensures training matches inference Stage 1 (which skips this entirely).
                noisy_zero = context_payload.get("noisy_group_zero_range")
                if noisy_zero is not None:
                    s_ng, e_ng = noisy_zero
                    tv_out_dyn[:, s_ng:e_ng, :] = 0
                # Zero cross-attn output for dropped task_video samples
                # to prevent o.bias leak when attention mask is all-False.
                tv_dropped = context_payload.get("task_video_dropped_mask")
                if tv_dropped is not None:
                    _keep = torch.tensor(
                        [not d for d in tv_dropped],
                        dtype=tv_out_dyn.dtype, device=tv_out_dyn.device,
                    ).view(-1, 1, 1)
                    tv_out_dyn = tv_out_dyn * _keep
                x = x + tv_out_dyn
            ...
            if context is not None:
                ...
                capture = (cross_attn_weights_store is not None)
                cross_out, weights = block.cross_attn(
                    block.norm3(x), context, ctx_mask=context_mask,
                    return_weights=capture,
                )
                x = x + cross_out
                if capture:
                    cross_attn_weights_store.append(weights)
    ...
    def _apply_post_with_optional_checkpoint(
        ...
        Args:
            ...
            cross_attn_weights_store: If not None, cross-attn weights are appended here.
                Disabled when checkpointing (checkpointed fn cannot return side outputs).
        ...
        def _post_fn(
            ...
            # cross_attn_weights_store always None inside checkpointed fn
        ...
        # Not checkpointing: weight capture is available
        return self._apply_expert_post_block(
            block=block,
            ...
            cross_attn_weights_store=cross_attn_weights_store,
        )
    ...
    def forward(
        ...
        prepend_cross_attn_slice: Optional[tuple] = None,
    ):
        ...
        if attention_mask.ndim == 2:
            ...
            if attention_mask.shape[1] != 1 or attention_mask.shape[2] != attention_mask.shape[3]:
                raise ValueError(
                    f"`attention_mask` 4D form must be [B, 1, S, S], got shape {tuple(attention_mask.shape)}"
                )
        else:
            raise ValueError(
        ...
        self._last_attn_weights = None          # set on last layer (self-attn)
        self._last_video_cross_attn_weights = None  # set on last layer (cross-attn, if captured)
        ...
        for layer_idx in range(self.num_layers):
            ...
            if is_last_layer and self.training:
                mixed, self._last_attn_weights = self._eager_attention_with_weights(
            ...
            # prepend_cross_attn: reset each layer before the slicing loop so action expert
            # always gets the current layer's video expert output (not a stale previous layer).
            dynamic_tv_ctx: Optional[torch.Tensor] = None
            ...
            for name, seq_len in zip(self.expert_order, seq_lens):
                ...
                # prepend_cross_attn: inject per-layer dynamic [p|tv] context into action expert.
                # video runs first in expert_order, so dynamic_tv_ctx is set before action.
                if dynamic_tv_ctx is not None and name == "action" and context_payload is not None:
                    context_payload = {**context_payload, "task_video_context": dynamic_tv_ctx}

                # Capture video cross-attn weights on the last layer (when not checkpointing)
                capture_store: Optional[list] = None
                if is_last_layer and name == "video" and self.training:
                    capture_store = []
                ...
                updated_tokens = self._apply_post_with_optional_checkpoint(
                    ...
                    context_payload=context_payload,
                    cross_attn_weights_store=capture_store,
                )

                if capture_store:
                    self._last_video_cross_attn_weights = capture_store[0]  # [B, H, S_vid, L+S_task]

                tokens_all[name] = updated_tokens

                # prepend_cross_attn: extract [p|tv] from video expert's post-MLP output
                if name == "video" and prepend_cross_attn_slice is not None:
                    s, e = prepend_cross_attn_slice
                    dynamic_tv_ctx = updated_tokens[:, s:e, :]  # [B, p+tv, D]
    ...
    def forward_single_expert(
        ...
        """Run a single expert through all layers without mixed attention with other experts.

        Uses the same decomposed processing as :meth:`forward` —
        :meth:`_build_expert_attention_io`, :meth:`_mixed_attention`, and
        :meth:`_apply_expert_post_block` — to ensure identical behavior.

        This is used for inference Stage 1 (progress denoising) where only the
        noisy group tokens are processed through the video expert blocks.

        Args:
            expert_name: Key in ``self.mixtures`` (e.g. ``"video"``).
            tokens: [B, S, D] input tokens.
            freqs: [S, 1, freq_dim] RoPE frequencies.
            t_mod: [B, S, 6, D] time modulation.
            context_payload: Cross-attention context dict (``context``, ``mask``, etc.).
            attention_mask: [S, S] boolean self-attention mask.

        Returns:
            tokens: [B, S, D] output tokens after all layers.
        """
        ...
        for layer_idx in range(self.num_layers):
            ...
            (
                ...
                use_gradient_checkpointing,
            ) = self._build_expert_attention_io(expert, block, tokens, freqs, t_mod)

            attn_out = self._mixed_attention(
                q_cat=q, k_cat=k, v_cat=v, attention_mask=attention_mask,
            )

            tokens = self._apply_post_with_optional_checkpoint(
                block=block,
                residual_x=residual_x,
                gate_msa=gate_msa,
                shift_mlp=shift_mlp,

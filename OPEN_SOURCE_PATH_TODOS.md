...
| `alignment/config.py` (`CONFIG.DATA.TASK_PATHS_TRANSFORMS`, esp. `"10042"` entry) | Mirrors policy_training's `PATH_TRANSFORMS` | Same treatment as `path_transforms_config.py` above. |
...
- **`policy_training/checkpoints` symlink** (→ internal `open_ckpts/fastwam_ckpts`): confirmed

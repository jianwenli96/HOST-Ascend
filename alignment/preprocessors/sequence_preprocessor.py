...
def add_additive_noise_to_sequence(tensor, noise_stddev, seed=None):
  ...
  return tensor + noise_stddev * tf.random.normal(
...
def largest_square_crop(image_size):
  ...
  height_offset = tf.cast((image_size[0] - min_dim) / 2, tf.int32)
  width_offset = tf.cast((image_size[1] - min_dim) / 2, tf.int32)
...
def random_square_crop(image_size, min_scale):
  ...
  """
  min_dim = tf.reduce_min(image_size[0:2])
  sampled_size = tf.cast(
      tf.cast(min_dim, tf.float32) * tf.random.uniform([], min_scale, 1.0), tf.int32)

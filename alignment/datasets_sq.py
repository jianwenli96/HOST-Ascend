...
class TCCCollator:
        ...
        for idx, p in enumerate(paths):
            ...
            if isinstance(p, str) and p.startswith("video://"):
                ...
                try:
                    ...
                    print(f"[TCCCollator load_imgs] ⚠️ 警告: 捕获到损坏的图片，已使用替身跳过: {p}. 错误: {e}")
                    # 核心容错逻辑：如果是视频连续帧坏了，直接用列表中已经成功读取的上一帧作为完美替身。
                    # 如果是极小概率第一帧就坏了，给一张符合尺寸要求的纯黑图。
                    if len(imgs) > 0:
                        img = imgs[-1].copy()
                    else:
                        img = Image.new('RGB', (CONFIG.IMAGE_SIZE, CONFIG.IMAGE_SIZE), (0, 0, 0))
    ...
    def _resolve_eval_M(self, batch, target_M, c_size, chunk_len):
        """Validate target_M against the first batch sample's video length.
        ...
        """
        multipliers = [1, 2, 4]
        num_frames = len(batch[0]['frame_paths']) // c_size
    ...
    def _build_qwen_input(self, batch, context_size,
        ...
        """Load images, optionally augment, and run Pass 1 + Pass 2.
        ...
        # ---- Preparation pass: load + augment all samples, cache for two-pass packing ----
        prepared = []
        for i in range(len(batch)):
            main_paths = batch[i].get(frame_paths_key, [])
            ref_paths  = batch[i].get(ref_frame_paths_key, [])
            ...
            ref_all  = self.load_imgs(align + ref_paths)
            ...
            ref_align  = ref_all[:a_len];   ref_flat  = ref_all[a_len:]
            ...
            prepared.append({
                ...
                'has_ref': bool(batch[i].get(ref_frame_paths_key)),
    ...
    def __call__(self, batch):
        ...
        # --- Phase 3: Student Qwen processing (augmented, possibly reversed frames) ---
        student_sink = self._build_qwen_input(
            ...
            align_paths_key='_cached_align_paths')
        collated_batch['qwen_input'] = self._assemble_qwen_dict(student_sink)
...
class LiberoDataset(Dataset):
    ...
    def _load_joint_data(self, video_info, dataset_name):
        ...
        T = episode length, D_joint = total active joint dims.
        """

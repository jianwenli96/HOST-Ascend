...
class SelfAttention(nn.Module):
    def __init__(self, hidden_dim: int, attn_head_dim: int, num_heads: int, eps: float = 1e-6,
        ...
        assert attn_gate_mode in ("none", "headwise", "elementwise", "value_headwise"), \
            f"attn_gate_mode must be 'none', 'headwise', 'elementwise', or 'value_headwise', got '{attn_gate_mode}'"
        self.attn_gate_mode = attn_gate_mode
        if attn_gate_mode in ("headwise", "value_headwise"):
            self.gate_proj = nn.Linear(hidden_dim, num_heads)
            nn.init.zeros_(self.gate_proj.weight)
            nn.init.constant_(self.gate_proj.bias, 5.0)
        elif attn_gate_mode == "elementwise":
            self.gate_proj = nn.Linear(hidden_dim, self.attn_hidden_dim)
            nn.init.zeros_(self.gate_proj.weight)
            nn.init.constant_(self.gate_proj.bias, 5.0)
        else:
            self.gate_proj = None
    ...
    def forward(self, x, freqs, self_attn_mask: Optional[torch.Tensor] = None):
        ...
        v = self.v(x)  # [B, S, H*Dh]
        if self.attn_gate_mode == "value_headwise":
            B, S, _ = v.shape
            gate = torch.sigmoid(self.gate_proj(x_input)).view(B, S, self.num_heads, 1)
            v = (v.view(B, S, self.num_heads, self.attn_head_dim) * gate).flatten(2)
        q = rope_apply(q, freqs, self.num_heads)
        k = rope_apply(k, freqs, self.num_heads)
        x = flash_attention(q=q, k=k, v=v, num_heads=self.num_heads, ctx_mask=self_attn_mask)
        # x: [B, S, H*Dh]
        if self.gate_proj is not None and self.attn_gate_mode != "value_headwise":
            B, S, _ = x.shape
            gate_score = self.gate_proj(x_input)  # [B, S, H] or [B, S, H*Dh]
            gate = torch.sigmoid(gate_score).view(B, S, self.num_heads, -1)  # [B, S, H, 1] or [B, S, H, Dh]
            x = (x.view(B, S, self.num_heads, self.attn_head_dim) * gate).flatten(2)
        return self.o(x)


class CrossAttention(nn.Module):
    def __init__(self, hidden_dim: int, attn_head_dim: int, num_heads: int, eps: float = 1e-6,
                 ctx_dim: Optional[int] = None, gate_mode: str = "none"):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.num_heads = num_heads
        self.attn_head_dim = attn_head_dim
        self.attn_hidden_dim = self.num_heads * self.attn_head_dim
        ctx_dim = ctx_dim if ctx_dim is not None else hidden_dim

        self.q = nn.Linear(hidden_dim, self.attn_hidden_dim)
        self.k = nn.Linear(ctx_dim, self.attn_hidden_dim)
        self.v = nn.Linear(ctx_dim, self.attn_hidden_dim)
        self.o = nn.Linear(self.attn_hidden_dim, hidden_dim)
        self.norm_q = RMSNorm(self.attn_hidden_dim, eps=eps)
        self.norm_k = RMSNorm(self.attn_hidden_dim, eps=eps)

        assert gate_mode in ("none", "headwise", "elementwise", "value_headwise"), \
            f"gate_mode must be 'none', 'headwise', 'elementwise', or 'value_headwise', got '{gate_mode}'"
        ...
        if gate_mode == "headwise":
            ...
            nn.init.zeros_(self.gate_proj.weight)
            nn.init.constant_(self.gate_proj.bias, 5.0)
        elif gate_mode == "value_headwise":
            self.gate_proj = nn.Linear(ctx_dim, num_heads)  # input from ctx (V source)
            nn.init.zeros_(self.gate_proj.weight)
            nn.init.constant_(self.gate_proj.bias, 5.0)
        else:
            self.gate_proj = None

    def _apply_gate(self, out: torch.Tensor, x_input: torch.Tensor) -> torch.Tensor:
        B, S, _ = out.shape
        gate_score = self.gate_proj(x_input)  # [B, S, H] or [B, S, H*Dh]
        gate = torch.sigmoid(gate_score).view(B, S, self.num_heads, -1)  # [B, S, H, 1] or [B, S, H, Dh]
        return (out.view(B, S, self.num_heads, self.attn_head_dim) * gate).flatten(2)
    ...
    def forward(
        ...
        q_freqs: Optional[torch.Tensor] = None,
        k_freqs: Optional[torch.Tensor] = None,
    ):
        x_input = x
        q = self.norm_q(self.q(x))   # [B, S, H*d]
        k = self.norm_k(self.k(ctx)) # [B, C, H*d]
        v = self.v(ctx)               # [B, C, H*d]
        if self.gate_mode == "value_headwise":
            B_v, C, _ = v.shape
            gate = torch.sigmoid(self.gate_proj(ctx)).view(B_v, C, self.num_heads, 1)
            v = (v.view(B_v, C, self.num_heads, self.attn_head_dim) * gate).flatten(2)
        if q_freqs is not None:
            q = rope_apply(q, q_freqs, self.num_heads)
        if k_freqs is not None:
            k = rope_apply(k, k_freqs, self.num_heads)
        if return_weights:
            B, S, _ = q.shape
            C = k.shape[1]
            q_ = q.reshape(B, S, self.num_heads, self.attn_head_dim).permute(0, 2, 1, 3)
            k_ = k.reshape(B, C, self.num_heads, self.attn_head_dim).permute(0, 2, 1, 3)
            v_ = v.reshape(B, C, self.num_heads, self.attn_head_dim).permute(0, 2, 1, 3)
            scale = self.attn_head_dim ** -0.5
            scores = (q_ @ k_.transpose(-2, -1)) * scale  # [B, H, S, C]
            if ctx_mask is not None:
                scores = scores.masked_fill(~ctx_mask.expand_as(scores), float('-inf'))
            weights = scores.softmax(dim=-1).nan_to_num(0.0)  # [B, H, S, C]
            out = (weights @ v_).permute(0, 2, 1, 3).reshape(B, S, self.attn_hidden_dim)
            if self.gate_proj is not None and self.gate_mode != "value_headwise":
                out = self._apply_gate(out, x_input)
            return self.o(out), weights.detach()
        out = flash_attention(q=q, k=k, v=v, num_heads=self.num_heads, ctx_mask=ctx_mask)
        if self.gate_proj is not None and self.gate_mode != "value_headwise":
            out = self._apply_gate(out, x_input)
...
class DiTBlock(nn.Module):
    def __init__(self,  hidden_dim: int, attn_head_dim: int, num_heads: int, ffn_dim: int, eps: float = 1e-6,
                 use_task_video_cross_attn: bool = False, task_video_ctx_dim: Optional[int] = None,
                 attn_gate_mode: str = "none", task_video_cross_attn_gate_mode: str = "none"):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.attn_head_dim = attn_head_dim
        self.num_heads = num_heads
        ...
        self.self_attn = SelfAttention(hidden_dim, attn_head_dim, num_heads, eps, attn_gate_mode=attn_gate_mode)
        ...
        if use_task_video_cross_attn:
            self.task_video_cross_attn = CrossAttention(
                hidden_dim, attn_head_dim, num_heads, eps, ctx_dim=task_video_ctx_dim,
                gate_mode=task_video_cross_attn_gate_mode)
            nn.init.zeros_(self.task_video_cross_attn.o.weight)
            nn.init.zeros_(self.task_video_cross_attn.o.bias)
            self.norm_task_video = nn.LayerNorm(hidden_dim, eps=eps)
        else:
            self.task_video_cross_attn = None
            self.norm_task_video = None
    ...
    def forward(self, x, context, t_mod, freqs, context_mask=None, self_attn_mask: Optional[torch.Tensor] = None):
        ...
        cross_out, _ = self.cross_attn(self.norm3(x), context, ctx_mask=context_mask)
        x = x + cross_out
...
class WanVideoDiT(torch.nn.Module):
    def __init__(
        ...
        use_task_video_cross_attn: bool = False,
        task_video_ctx_dim: Optional[int] = None,
        attn_gate_mode: str = "none",
        task_video_cross_attn_gate_mode: str = "none",
    ):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.in_dim = in_dim
        ...
        self.blocks = nn.ModuleList([
            DiTBlock(hidden_dim, attn_head_dim, num_heads, ffn_dim, eps,
                     use_task_video_cross_attn=use_task_video_cross_attn,
                     task_video_ctx_dim=task_video_ctx_dim,
                     attn_gate_mode=attn_gate_mode,
                     task_video_cross_attn_gate_mode=task_video_cross_attn_gate_mode)
